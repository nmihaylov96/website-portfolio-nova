...
const projectPreviews = [
  ...
  {
  title: "Elysian",
  labelKey: "work.p3.label",
  descKey: "work.p3.desc",
  img: "https://image.thum.io/get/width/1200/crop/630/noanimate/https://elysian-realestates.vercel.app",
  url: "https://elysian-realestates.vercel.app",
  color: "from-emerald-500/20",
  tags: ["React", "TypeScript", "Tailwind", "Framer Motion"],
  stat: "Luxury Real Estate",
  },
];
  ...
  return (
    <div className="relative bg-[#0d1424] text-white overflow-x-hidden">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        ...
        <div
          className="animated-blob bg-violet-600/10 w-[500px] h-[500px] top-[40%] right-[-150px]"
          style={{ animationDelay: "-7s" }}
        />
        <div
          className="animated-blob bg-indigo-500/8 w-[600px] h-[600px] bottom-[-200px] left-[20%]"
      ...
      <section className="relative z-10 min-h-screen flex flex-col justify-center pt-28 pb-16 px-6 md:px-16 max-w-7xl mx-auto">
        <motion.div variants={stagger} initial="hidden" animate="visible">
          ...
          <motion.div
            variants={fadeUp}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-card text-xs font-semibold tracking-widest uppercase mb-10 text-primary w-fit"
          >
          ...
          >
            {[
              ...
              {
                v: t("hero.stat.projects.value"),
                l: t("hero.stat.projects.label"),
              },
              {
                v: t("hero.stat.clients.value"),
                l: t("hero.stat.clients.label"),
              },
              { v: t("hero.stat.score.value"), l: t("hero.stat.score.label") },
            ].map(({ v, l }) => (
              <div key={l} className="glass-card px-5 py-4 rounded-2xl">
                <div className="text-2xl md:text-3xl font-black text-gradient-primary mb-0.5">
                  {v}
                </div>
      ...
      <div className="relative z-10 py-5 border-y border-white/5 overflow-hidden bg-white/[0.02]">
        <div className="marquee-track flex gap-0">
          {[...marqueeItems, ...marqueeItems].map((item, i) => (
            <div
              key={i}
              className="marquee-item flex items-center gap-3 px-6 shrink-0"
            >
      ...
      <section className="relative z-10 py-28 px-6 md:px-16 max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-12 md:gap-20 items-center">
            ...
            <div className="relative rounded-3xl overflow-hidden aspect-square glass-card p-1">
              <div className="w-full h-full rounded-2xl bg-gradient-to-br from-primary/20 via-secondary/10 to-transparent flex items-center justify-center relative overflow-hidden">
                ...
                <div className="absolute bottom-6 right-6 text-right">
                  <div className="text-5xl font-black text-gradient-primary">
                    NOVA
                  </div>
          ...
          >
            ...
            <motion.h2
              variants={fadeUp}
              className="text-3xl md:text-4xl font-black leading-tight text-gradient"
            >
              {t("about.title")}
            </motion.h2>
            ...
            <motion.div variants={fadeUp} className="flex flex-wrap gap-2 pt-2">
              {[t("about.badge1"), t("about.badge2"), t("about.badge3")].map(
                (b) => (
                  <span
                    key={b}
                    className="px-4 py-1.5 rounded-full glass-card text-sm font-medium border border-white/10"
                  >
                    {b}
                  </span>
                ),
      ...
      <section className="relative z-10 py-28 px-6 md:px-16 max-w-7xl mx-auto">
        ...
        >
          ...
          <motion.h2
            variants={fadeUp}
            className="text-4xl md:text-6xl font-black tracking-tight text-gradient mb-4"
          >
            {t("featured.title")}
          </motion.h2>
          <motion.p
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-12">
          {projectPreviews.map(
            ({ title, labelKey, descKey, img, url, color, tags, stat }, i) => (
              <motion.div
                key={title}
                initial="hidden"
                whileInView="visible"
              ...
              >
                {/* Image */}
                <div
                  className={`relative overflow-hidden bg-gradient-to-br ${color} to-transparent p-1`}
                >
                  <div className="rounded-2xl overflow-hidden relative">
                    <img
                      src={img}
                      alt={title}
                      className="w-full h-auto grayscale-[10%] group-hover:grayscale-0 group-hover:scale-[1.02] transition-all duration-700"
                    />
      ...
      <section className="relative z-10 py-28 px-6 md:px-16 max-w-7xl mx-auto">
        ...
        >
          <motion.div
            variants={fadeUp}
            className="text-xs font-bold uppercase tracking-widest text-primary flex items-center justify-center gap-2 mb-4"
          >
            <Sparkles className="w-3.5 h-3.5" />
            {t("services.label")}
          </motion.div>
          <motion.h2
            variants={fadeUp}
            className="text-4xl md:text-6xl font-black tracking-tight text-gradient"
          >
            {t("services.title")}
          </motion.h2>
        </motion.div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {services.map(({ icon, titleKey, descKey, color, bg }, i) => (
            ...
            >
              <div
                className={`${color} group-hover:scale-110 transition-transform duration-300 w-fit`}
              >
                {icon}
              </div>
              <div>
        ...
        >
          <div className="relative rounded-3xl overflow-hidden p-1">
            ...
            <div className="relative glass-card rounded-[calc(1.5rem-4px)] p-12 md:p-20">
              <motion.h2
                variants={fadeUp}
                className="text-4xl md:text-6xl font-black tracking-tight mb-6 text-gradient"
              >
                {t("home.cta.title")}
              </motion.h2>
              <motion.p
